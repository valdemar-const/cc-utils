...
class CC_RUNTIME_API graph {
 public:
  ...
  auto remove_edge(std::string_view src_node, std::string_view src_slot,
  ...
  auto nodes() const -> std::span<const std::unique_ptr<node>> { return nodes_; }
  auto edges() const -> std::span<const edge>                   { return edges_; }
