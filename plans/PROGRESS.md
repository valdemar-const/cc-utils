...
- `docs/pipeline-format.md` — каноническая спецификация формата: термины
  (две среды, контракт, замыкание), XML-схема v2, версионирование,
  правила контракта доменов, идентичность узлов, таксономия компонент
  (§7: pipeline-плагины vs workbench-плагины vs тип-пакеты/библиотеки,
  матрица видимости), отказоустойчивость загрузки
- `View → Open tab → Metadata` открывает статический DockableWindow в
  MainDockSpace: (1) контракт документа — root (иммутабелен) + imports:
  добавление свободное (любой домен кроме root/явных дубликатов,
  транзитивные deps можно закрепить явно), удаление — по кнопке, пока
  после сжатия замыкания все поставленные узлы остаются видимыми (точная
  проверка с tooltip об orphan-узлах), (2) регистрация кастомного домена
  в рантайме (id-валидация `[a-z0-9/._-]`, deps-чекбоксы; session-only,
  не персистится в файл), (3) обзор реестра: все домены с description/
  depends_on/provided_types/узлами-членами, (4) загруженные плагины со
  счётчиком узлов
