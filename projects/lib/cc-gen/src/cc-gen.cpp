...
const char* reg_name(cc::nasm::reg r) {
  switch (r) {
    case cc::nasm::reg::rax: return "rax";
    case cc::nasm::reg::rdi: return "rdi";
  }
  return "?";
}
...
std::vector<cc::nasm::instr> lower(const cc::ir::module& mod) {
...
std::string format(const std::vector<cc::nasm::instr>& instrs) {
  std::string s;
  s += "global _start\nsection .text\n_start:\n";
  for (const auto& i : instrs) {
    ...
    switch (i.mn) {
      case cc::nasm::mnemonic::mov: {
        s += "mov ";
        if (i.ops[0].kind == cc::nasm::operand_kind::reg) s += reg_name(i.ops[0].reg);
        s += ", ";
