...
auto
runner::pull(std::string_view node_id, std::string_view slot_id)
        -> std::expected<const any_value *, failure>
{
    ...
    if (!target)
    {
        return std::unexpected(failure {"node " + std::string {node_id} + " has no slot '" + std::string {slot_id} + "'"});
    }
    ...
    {
        return std::unexpected(failure {"node " + std::string {node_id} + " has no output slot '" + std::string {slot_id} + "'"});
    }
    return std::addressof(inner->second);
}

auto
runner::ensure_outputs(std::string_view node_id) -> std::expected<void, failure>
{
    ...
    for (auto *s : n->slots())
    {
        ...
        if (!src)
        {
            ...
            return std::unexpected(failure {"unconnected input '" + std::string {s->id()} + "' on node " + key});
        }
    ...
    auto result = n->activate(inputs, outputs, ctx_);
