...
struct ir_emitter : cc::ast::visitor {
  ...
  void visit(const cc::ast::program& p) override {
    for (const auto& stmt : p.body) stmt->accept(*this);
  }
  void visit(const cc::ast::return_stmt& r) override {
    if (r.value) r.value->accept(*this);  // resolves pending_imm via visit(int_literal)
    mod.code.push_back({cc::ir::opcode::ret, pending_imm});
  }
  void visit(const cc::ast::int_literal& l) override { pending_imm = l.value; }
};
...
cc::ir::module lower(const cc::ast::program& root) {
  ir_emitter e;
  root.accept(e);
  return e.mod;
}
