...
class type_registry_impl final : public type_registry {
 public:
  auto name_of(type_descriptor_t d) const -> std::string_view override {
    auto it = desc_to_name_.find(d);
    return it == desc_to_name_.end() ? std::string_view{} : it->second;
  }

  auto descriptor_of_name(std::string_view name) const -> type_descriptor_t override {
    auto it = name_to_desc_.find(std::string{name});
    return it == name_to_desc_.end() ? type_descriptor_t{} : it->second;
  }

  auto is_connectable(type_descriptor_t out, type_descriptor_t in) const -> bool override {
    if (out == in) return true;
    if (any_descriptor_ && in == *any_descriptor_) return true;
    return false;
  }

  auto register_value_type_impl(std::string_view name, type_descriptor_t d) -> bool override {
    if (name == "any") {
      if (any_descriptor_ && *any_descriptor_ != d) return false;
      any_descriptor_ = d;
    }
    auto [it_n, inserted_n] = name_to_desc_.try_emplace(std::string{name}, d);
    if (!inserted_n) return it_n->second == d;  // idempotent ok, mismatch fails
...
class view_renderer_provider_impl final : public view_renderer_provider {
 public:
  explicit view_renderer_provider_impl(const type_registry& types) : types_{types} {}

  auto get_for_type(type_descriptor_t type) const -> view_renderer* override {
  ...
  auto all() const -> std::span<view_renderer* const> override { return order_; }

  auto register_renderer(std::unique_ptr<view_renderer> r) -> void override {
    if (!r) return;
...
class host_registry_impl final : public host_registry {
 public:
  host_registry_impl()
      : types_{}, renderers_{types_} {}

  auto types() -> type_registry& override              { return types_; }
  auto types() const -> const type_registry& override  { return types_; }

  auto register_node_factory(std::unique_ptr<node_factory> factory) -> void override {
    if (!factory) return;
  ...
  auto node_factories() const -> std::span<node_factory* const> override {
...
auto make_host_registry() -> std::unique_ptr<host_registry> {
