...
auto append_attribute(pugi::xml_node node, const char* name, std::string_view value) -> void {
  // pugixml wants a null-terminated string; build one explicitly.
  node.append_attribute(name).set_value(std::string{value}.c_str());
}

auto append_float_attribute(pugi::xml_node node, const char* name, float v) -> void {
...
auto parse_float_attr(const pugi::xml_attribute& a, float& out) -> bool {
...
auto collect_required_plugins(const host_registry& host, const runtime::graph& g)
  ...
  for (auto const& n : g.nodes()) {
    ...
    if (!provider.empty()) uniq.emplace(provider);
  }
  return {uniq.begin(), uniq.end()};
}

auto write_requires(pugi::xml_node root, const std::vector<std::string>& plugins) -> void {
  ...
  for (auto const& n : g.nodes()) {
    ...
    if (auto* f = host.find_node_factory(n->type_id())) {
      for (auto const& desc : f->property_schema()) {
        std::string_view value = n->properties().get(desc.key);
        auto prop_el = props_el.append_child("property");
        append_attribute(prop_el, "key", desc.key);
        prop_el.append_child(pugi::node_pcdata).set_value(std::string{value}.c_str());
      }
    }
  }
}

auto write_edges(pugi::xml_node root, const runtime::graph& g) -> void {
...
auto read_version(const pugi::xml_document& doc) -> std::expected<int, std::string> {
...
auto check_requires(const host_registry& host, const pugi::xml_node& root, load_warnings& w)
    -> void {
  auto req = root.child("requires");
  if (!req) return;
  // Build a lookup set of currently-loaded plugin names for O(1) test.
  std::set<std::string> loaded;
  for (auto const& name : host.loaded_plugins()) loaded.emplace(name);
...
auto read_positions(const pugi::xml_node& node_el, const std::string& instance_id,
                    std::unordered_map<std::string, pos>& out) -> void {
...
auto load_pipeline(const host_registry& host,
  ...
  pugi::xml_document doc;
  pugi::xml_parse_result parse_res = doc.load_file(path.c_str(), pugi::parse_default,
                                                   pugi::encoding_utf8);
  if (!parse_res) {
    return std::unexpected<std::string>(std::string{"XML parse error: "}
                                        + parse_res.description()
                                        + " (offset " + std::to_string(parse_res.offset) + ")");
  }
  ...
  if (*version_res != k_pipeline_format_version) {
    ...
    return std::unexpected<std::string>("unsupported pipeline version "
                                        + std::to_string(*version_res)
                                        + " (expected "
                                        + std::to_string(k_pipeline_format_version) + ")");
  }
  ...
  // nodes populated below; on early-return error paths the graph stays empty.
  {
    ...
    for (auto const& n : g.nodes()) ids.emplace_back(n->instance_id());
  ...
  if (auto nodes = root.child("nodes")) {
    for (auto node_el = nodes.child("node"); node_el; node_el = node_el.next_sibling("node")) {
      ...
      if (!type_id || !*type_id || !inst_id || !*inst_id) continue;
  ...
  if (auto edges = root.child("edges")) {
    for (auto edge_el = edges.child("edge"); edge_el; edge_el = edge_el.next_sibling("edge")) {
      ...
      if (created_ids.find(src_node) == created_ids.end() ||
          created_ids.find(dst_node) == created_ids.end()) {
